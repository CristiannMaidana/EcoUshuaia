...
class CustomDatosUsuario extends StatefulWidget {
  const CustomDatosUsuario({super.key});

  @override
  State<CustomDatosUsuario> createState() => _CustomDatosUsuarioState();
}

class _CustomDatosUsuarioState extends State<CustomDatosUsuario>
    with SingleTickerProviderStateMixin {
  final bool _historial = false; // Simula si hay historial de reciclaje o no

  @override
  Widget build(BuildContext context) {
    final usuarioVm = context.watch<UsuarioViewModel>();
    final domicilioVm = context.watch<DomicilioViewModel>();
    final usuario = usuarioVm.usuario;
    final nombreCompleto =
        usuario?.nombreCompleto ??
        (usuarioVm.loading ? 'Cargando perfil...' : 'Usuario');
    final domicilio = domicilioVm.domicilio;
    final direccion = domicilio != null
        ? [
            domicilio.calle,
            domicilio.numero,
            domicilio.ciudad,
                    ...
                    onPressed: usuario == null
                        ...
                        : () {
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) =>
                                    EditUserScreen(initialUser: usuario),
                              ),
                            );
                          },
