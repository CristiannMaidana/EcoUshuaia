...
class CalendarioDto {
  ...
  static Duration _parseIntervalToDuration(String s) {
    ...
    final days = match.group(1) != null ? int.parse(match.group(1)!) : 0;
    final hh = int.parse(match.group(2)!);
    final mm = int.parse(match.group(3)!);
    final ss = int.parse(match.group(4)!);

    final microStr = match.group(5);
    final micros = microStr == null
        ? 0
        : int.parse(
            microStr.length >= 6
                ? microStr.substring(0, 6)
                : microStr.padRight(6, '0'),
          );
