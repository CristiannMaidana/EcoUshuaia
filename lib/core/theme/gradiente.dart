...
class Gradiente extends StatelessWidget {
  ...
  const Gradiente({super.key, this.height = 220, this.borderRadius});

  @override
  Widget build(BuildContext context) {
    const baseA = Color.fromARGB(255, 208, 242, 226); // izquierda (mint té)
    const baseB = Color.fromARGB(
      255,
      146,
      240,
      188,
    ); // derecha (verde suave, un poco más “presente”)
    const radialA = Color(0xFFE6FBF1); // realce abajo-izq
    const radialB = Color.fromARGB(
      95,
      255,
      255,
      255,
    ); // realce arriba-der (reemplaza el azul por verde agua)

    Widget layer(Gradient g) => Positioned.fill(
      child: DecoratedBox(
        decoration: BoxDecoration(gradient: g, borderRadius: borderRadius),
      ),
    );
